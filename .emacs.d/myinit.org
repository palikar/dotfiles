...
(set-fill-column 80)
(define-key my-keys-mode-map (kbd "M-<f8>") 'fci-mode)
...
#+END_SRC
** Custom key bindings(some of them!) 
#+BEGIN_SRC emacs-lisp
...
(define-key my-keys-mode-map (kbd "C-x c C-a") 'helm-apt) ;;search and install apt packages through Emacs...crazy, I know!!!
...
;; (define-key my-keys-mode-map (kbd "C-x r b") 'helm-register)
...
#+BEGIN_SRC emcas-lisp :results none
(require 'diminish)
#+END_SRC
** Diminishing modes
...
** TODO Create a separate section with all of the defined functions
