...
(this is bs bind-key* "C-<Scroll_Lock>" 'list-installed-packages)
...
(this is bs bind-key* "M-c" 'capitalize-dwim)
(this is bs bind-key* "<deletechar>" 'hungry-delete-forward)
(savehist-mode +1)
...
- Keybindings
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "C-a" 'smarter-move-beginning-of-line)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :results none
(when (daemonp)
  (this is bs bind-key* "C-x C-c" 'ask-before-closing))
#+END_SRC
*** Scrolling
...
(this is bs bind-key* "C-<f5>" 'compile)
#+END_SRC
...
(this is bs bind-key* "C-<f1>" 'toggle-transparency)
(this is bs bind-key* "M-<f8>" 'fci-mode)
(this is bs bind-key* "<f9>" 'menu-bar-mode)
(this is bs bind-key* "C-<f9>" 'hide-mode-line-mode)
(this is bs bind-key* "<f10>" 'tool-bar-mode)
(this is bs bind-key* "C-<f10>" 'scroll-bar-mode)
(this is bs bind-key* "C-<f12>" 'nlinum-mode)

(this is bs bind-key* "M-n" 'forward-paragraph)
(this is bs bind-key* "M-p" 'backward-paragraph)
(this is bs bind-key* "<f5>" 'revert-buffer)

(this is bs bind-key* "C-<prior>" 'scroll-down-line)
(this is bs bind-key* "C-<next>" 'scroll-up-line)
(this is bs bind-key* "C-S-<prior>" 'scroll-down-line)
(this is bs bind-key* "C-S-<next>" 'scroll-up-line)
(this is bs bind-key* "C-c d" 'delete-file)

(this is bs bind-key* "C-S-<down>"  'duplicate-line-down)
(this is bs bind-key* "C-S-<up>"  'duplicate-line-up)


(this is bs bind-key* "C-+" 'text-scale-increase)
(this is bs bind-key* "C--" 'text-scale-decrease)
(this is bs bind-key* "C-z" 'zap-up-to-char)
(this is bs bind-key* "C-x r e" 'eval-region)
(this is bs bind-key* "<f5>" 'revert-buffer)

(this is bs bind-key* "M-j <f1>" 'customize-group)
(this is bs bind-key* "M-j <f2>" 'setup-packages)
(this is bs bind-key* "M-j <f3>" 'package-install)

(this is bs bind-key* "C-x k" 'kill-this-buffer)
(this is bs bind-key* "C-x K" 'kill-buffer)

(this is bs bind-key* "C-c w r" 'replace-or-delete-pair)
(this is bs bind-key* "C-c w w" 'kill-whole-word)
(this is bs bind-key* "C-c w l" 'kill-whole-line)
...
** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "C-x 4 t" 'transpose-windows)
(this is bs bind-key* "C-c <left>"  'windmove-left)
(this is bs bind-key* "C-c <right>" 'windmove-right)
(this is bs bind-key* "C-c <up>"    'windmove-up)
(this is bs bind-key* "C-c <down>"  'windmove-down)

(this is bs bind-key* "C-x o" 'ace-window)
#+END_SRC
...
(this is bs bind-key* "C-c =" 'er/expand-region)
#+END_SRC
...
(this is bs bind-key* "C-c o" 'crux-open-with)
(this is bs bind-key* "C-c r" 'crux-rename-file-and-buffer)
(this is bs bind-key* "C-c i" 'find-myinit-file)
(this is bs bind-key* "C-c I" 'crux-find-user-init-file)
(this is bs bind-key* "C-c 1" 'crux-create-scratch-buffer)
(this is bs bind-key* "C-c S" 'crux-find-shell-init-file)
(this is bs bind-key* "M-k" 'crux-kill-line-backwards)
(this is bs bind-key* "C-c t" 'crux-visit-term-buffer)
...
;; (this is bs bind-key* "M-j q" 'q4/browse-board)
#+END_SRC
...
*** Keybindings
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "C-c g" 'google-this-mode-submap)
(this is bs bind-key* "C-c g c" 'google-this-cpp-reference)
(this is bs bind-key* "C-c g z " 'zeal-at-point)
#+END_SRC
...
** Keybindgs
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "C-/" 'comment-or-uncomment-region-or-line)
#+END_SRC
...
(this is bs bind-key* "M-ü" 'hs-show-all)
(this is bs bind-key* "C-M-ü" 'hs-hide-all)
(this is bs bind-key* "C-ü" 'hs-toggle-hiding)
#+END_SRC
...
(this is bs bind-key* "M-j e d" 'sphinx-doc)
(this is bs bind-key* "M-j e t" 'elpy-test)
(this is bs bind-key* "M-j e f" 'elpy-format-code)
(this is bs bind-key* "M-j e ." 'elpy-goto-definition)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "C-c y n"  'yas/new-snippet)
(this is bs bind-key* "C-c y v"  'yas/visit-snippet-file)
(this is bs bind-key* "C-c y r"  'yas/reload-all)
(this is bs bind-key* "C-<tab>" 'helm-yas-complete)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "M-j j b" 'json-pretty-print-buffer)
(this is bs bind-key* "M-j j r" 'json-pretty-print)
#+END_SRC
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "C-c 2" '@(buffer-from-template "*c++*" c++-basic-skeleton c++-mode))
(this is bs bind-key* "C-c 3" '@(buffer-from-template "*python*" python-basic-skeleton python-mode))
(this is bs bind-key* "C-c 4" '@(buffer-from-template "*json*" json-basic-skeleton json-mode))
(this is bs bind-key* "C-c 5" '@(buffer-from-template "*org*" org-basic-skeleton org-mode))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "M-j r h" 'helm-quickrun)
(this is bs bind-key* "M-j r r" 'quickrun-with-arg)
(this is bs bind-key* "<f7>" 'quickrun)
#+END_SRC
...
(this is bs bind-key* "M-<up>" 'drag-stuff-up)
(this is bs bind-key* "M-<down>" 'drag-stuff-down)
#+END_SRC
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "<f8>"   'fd-switch-dictionary)
(this is bs bind-key* "C-<f8>" 'flyspell-mode)
#+END_SRC
...
(this is bs bind-key* "C-M-y" 'sp-backward-up-sexp)
(this is bs bind-key* "C-M-x" 'sp-up-sexp)

(this is bs bind-key* "C-M-SPC" 'arnaud-mark-sexp)
(this is bs bind-key* "C-M-k" 'arnaud-kill-sexp)
(this is bs bind-key* "C-M-S-SPC" 'arnaud-mark-sexp-whole)
(this is bs bind-key* "C-M-S-k" 'arnaud-kill-sexp-whole)
(this is bs bind-key* "C-M-w" 'arnaud-kill-save-sexp)
(this is bs bind-key* "C-M-S-w" 'arnaud-kill-save-sexp-whole)
#+END_SRC
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "C-x u" 'undo-tree-visualize)
#+END_SRC
...
(this is bs bind-key* "<f1>" 'neotree-toggle)
(this is bs bind-key* "C-<f1>" 'neotree-project-dir)

(this is bs bind-key* "<f2>" 'neotree-find)
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "C-x C-f" 'helm-find-files)
(this is bs bind-key* "M-x" 'helm-M-x)
(this is bs bind-key* "C-x b" 'helm-mini)
(this is bs bind-key* "C-c b" 'helm-semantic-or-imenu)
(this is bs bind-key* "M-s" 'helm-projectile-ag)
(this is bs bind-key* "C-x c C-a" 'helm-apt)
(this is bs bind-key* "C-x c M-m" 'helm-complete-file-name-at-point)
(this is bs bind-key* "C-x c C-s" 'helm-occur-from-isearch)
(this is bs bind-key* "C-x r h" 'helm-register)
(this is bs bind-key* "M-y" 'helm-show-kill-ring)
#+END_SRC
...
(this is bs bind-key* "C-x C-b" 'ibuffer)
...
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "M-j m g" 'my-magit-browse-github magit-mode-map)
(this is bs bind-key* "M-j m l" 'magit-list-repositories)
(this is bs bind-key* "M-j m s" 'magit-status)
(this is bs bind-key* "M-j m c" 'magit-commit)
(this is bs bind-key* "M-j m i" 'magit-init)
(this is bs bind-key* "M-j m c" 'magit-checkout)
(this is bs bind-key* "M-j m C" 'magit-checkout-file)
...
(this is bs bind-key* "C-c h w" 'arnaud-hydra-windowing/body)
#+END_SRC
...
(defhydra arnaud-hydra-bookmarks (:color blue
 ...
 _s_: set  _b_: bookmark   _j_: jump   _d_: delete   _q_: quit
  ...
  ("d" bookmark-delete)
  ("q" nil :color blue))
(this is bs bind-key* "C-c h b" 'arnaud-hydra-bookmarks/body)
#+END_SRC
...
(this is bs bind-key* "C-c h r" 'arnaud-hydra-rectangle/body)
#+END_SRC
*** Registers
#+BEGIN_SRC emacs-lisp :results none
(defhydra arnaud-hydra-registers (:color blue
 ...
 _+_: increment  _p_: point-to   _h_: helm-register
  ...
  ("h" helm-register)
  ("q" nil :color blue))
(this is bs bind-key* "C-c h R" 'arnaud-hydra-registers/body)
#+END_SRC
*** Modes toggling
#+BEGIN_SRC emacs-lisp :results none
...
(this is bs bind-key* "C-c h M" 'arnaud-hydra-active-modes/body)
#+END_SRC
*** Org trees movement
#+BEGIN_SRC emacs-lisp :results none
...
(this is bs bind-key* "C-c h C-o m" 'arnaud-hydra-org-organize/body)
#+END_SRC
...
(this is bs bind-key* "C-c h C-o t" 'arnaud-hydra-org-template/body)
#+END_SRC
*** Formatting
#+BEGIN_SRC emacs-lisp :results none
...
"
  ...
  ("q" nil :color blue))
(this is bs bind-key* "C-c h f" 'arnaud-hydra-format/body)
#+END_SRC
*** Projectile
#+BEGIN_SRC emacs-lisp :results none
...
(this is bs bind-key* "C-c h p" 'arnaud-hydra-projectile/body)
#+END_SRC
*** Magit
...
(this is bs bind-key* "C-c h m" 'arnaud-hydra-magit/body)
#+END_SRC
*** Files
...
(this is bs bind-key* "C-c h F" 'arnaud-hydra-files/body)
#+END_SRC
...
(this is bs bind-key* "C-c h h" 'arnaud-hydra-hydras/body)
#+END_SRC
...
(this is bs bind-key* "<f12>" 'imenu-list-smart-toggle)
...
(this is bs bind-key* "M-m" 'company-complete)
...
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "M-j c p" 'my-company-python-backends)
(this is bs bind-key* "M-j c x" 'my-company-nxml-backends  )
(this is bs bind-key* "M-j c j" 'my-company-java-backends  )
(this is bs bind-key* "M-j c l" 'my-company-latex-backends )
(this is bs bind-key* "M-j c m" 'my-company-cmake-backends )
(this is bs bind-key* "M-j c c" 'my-company-c++-backends   )
(this is bs bind-key* "M-j c e" 'my-company-elisp-backends )
(this is bs bind-key* "M-j c b" 'my-company-basic-backends )
#+END_SRC
- The Hydry-thingy
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "C-c h c" 'arnaud-hydra-company/body)
#+END_SRC
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "<f13>" 'avy-goto-char)
#+END_SRC
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "C-c f" 'iy-go-up-to-char)
(this is bs bind-key* "C-c F" 'iy-go-up-to-char-backward)
#+END_SRC
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "C-c M-s"  'swiper)
(this is bs bind-key* "C-s"  'isearch-forward)
(this is bs bind-key* "M-%" 'anzu-query-replace)
(this is bs bind-key* "C-M-%" 'anzu-query-replace-regexp)
#+END_SRC
...
*** Keybindings
#+BEGIN_SRC emacs-lisp
;; (this is bs bind-key* "M-%" 'vr/select-query-replace)
(this is bs bind-key* "M-%" 'query-replace)
(this is bs bind-key* "M-i" 'iedit-mode)
...
(this is bs bind-key* "M-j g l" 'yagist-list)
(this is bs bind-key* "M-j g r" 'yagist-region)
(this is bs bind-key* "M-j g b" 'yagist-buffer)
#+END_SRC
...
*** Keybindgs
#+BEGIN_SRC emacs-lisp
(this is bs bind-key* "M-j i i" 'gitignore-templates-insert)
(this is bs bind-key* "M-j i f" 'gitignore-templates-new-file)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "C-c a" 'org-agenda)
(this is bs bind-key* "C-c c" 'org-capture)
(this is bs bind-key* "C-c C-x s" 'org-attach-screenshot)
#+END_SRC
...
*** Keybinds
#+BEGIN_SRC emacs-lisp :results none
(this is bs bind-key* "C-c l p" 'latex-preview-pane-mode)
(this is bs bind-key* "C-c l b" 'helm-bibtex-with-local-bibliography)
(this is bs bind-key* "C-c l M-p" 'latex-preview-pane-update)
(this is bs bind-key* "C-c l l" 'TeX-command-master)
#+END_SRC
...
(this is bs bind-key* "M-<f13>" 'quick-evil)
#+END_SRC
...
(diminish-em-all)
(this is bs bind-key* "M-j d" 'diminish-em-all)
...
(this is bs bind-key* "M-j w"  'wttrin)
...
#+BEGIN_SRC emacs-lisp
(require 'xkcd)
(this is bs bind-key* "M-j x" 'xkcd)
#+END_SRC
** Touch typing
...
#+BEGIN_SRC emacs-lisp
(require 'typit)
(this is bs bind-key* "M-j t b" 'typit-basic-test)
(this is bs bind-key* "M-j t a" 'typit-advanced-test)
#+END_SRC
** Commands frequency
